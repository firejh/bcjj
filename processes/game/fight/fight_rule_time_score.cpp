...
FightRuleTimeScore::FightRuleSingleFail(uint64_t fightId, FightRule2 rule2, uint64_t parentId): FightRule(fightId, rule2, parentId)
{}

bool FightRuleTimeScore::start()
{
    //排位赛开打之后,直接开始，没有任何限制，等待参赛人员加入匹配池即可
    return true;
}

bool FightRuleTimeScore::over()
{
    //时间到了立刻结束，跟规则无关
    return true;
}
...
bool FightRuleTimeScore::createChallData(std::vector<uint64_t>& validObjs)
{
    return true;
}

void FightRuleTimeScore::fourPkChallData(std::vector<uint64_t> objs)
{

}

void FightRuleTimeScore::contrl()
{
    ...
    //匹配对手并生成对战
    //乱序
    //std::random_shuffle(m_tempReady);
    while(m_tempReady.size() >= 2)
    {
        //获得obj1
        uint64_t objId1 = *(m_tempReady.begin());
        m_tempReady.erase(m_tempReady.begin());
        auto it = m_challDatas.find(objId1);
        //obj1是否已经存在战斗
        if(it != m_challDatas.end())
        {
            continue;
        }
        //获得obj2
        uint64_t objId2 = *(m_tempReady.begin());
        m_tempReady.erase(m_tempReady.begin());

        //生成的对战插入对战管理
        auto& challData = m_challDatas[objId1];//小的id作为key
        challData.objId1 = objId1;
        challData.objId2 = objId2;
    }
...
void FightRuleTimeScore::stopRule()
{
...
std::vector<uint64_t> FightRuleTimeScore::getTop(uint32_t num)
{
...
void FightRuleTimeScore::fightResult(uint64_t objId, User::Ptr user, uint8_t result, const std::string& pic)
{
...
uint32_t FightRuleTimeScore::getPositionVal(uint32_t num)
{
    if(num == 0)
        return num;
...
bool FightRuleTimeScore::bioOver(uint32_t value1, uint32_t value2)
{
